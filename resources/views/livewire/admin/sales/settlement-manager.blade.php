            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 relative overflow-hidden group">
                    ...
                    <div class="relative">
                        ...
                        <h3 class="text-3xl font-black text-slate-900">S/ {{ number_format($activeSession->starting_cash ?? 0, 2) }}</h3>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 relative overflow-hidden group">
                    ...
                    <div class="relative">
                        ...
                        <h3 class="text-3xl font-black text-emerald-600">S/ {{ number_format($$totalSales ?? 0, 2) }}</h3>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 relative overflow-hidden group">
                    ...
                    <div class="relative">
                        ...
                        <h3 class="text-3xl font-black text-rose-600">S/ {{ number_format($$totalExpenses ?? 0, 2) }}</h3>
                    </div>
                </div>
                ...
                <div class="bg-[#043d8a] p-6 rounded-[2rem] shadow-xl shadow-blue-900/20 relative overflow-hidden group">
                    ...
                    <div class="relative">
                        ...
                        <h3 class="text-3xl font-black text-white">S/ {{ number_format($$currentBalance ?? 0, 2) }}</h3>
                    </div>
                </div>
                ...
                <div class="bg-amber-50 p-6 rounded-[2rem] border border-amber-100 shadow-xl shadow-amber-200/50 relative overflow-hidden group">
                    ...
                    <div class="relative">
                        ...
                        <h3 class="text-3xl font-black text-amber-700">S/ {{ number_format($$totalCollections ?? 0, 2) }}</h3>
                    </div>
                </div>
            </div>
                    ...
                    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden">
                        ...
                        <div class="p-8">
                            ...
                            <div class="mb-8 p-6 bg-slate-50/50 rounded-2xl border border-slate-100">
                                <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Servicios a Cobrar</h3>
                                
                                <div class="flex flex-wrap items-end gap-4 mb-4">
                                    <div class="flex-1 min-w-[200px]">
                                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Servicio</label>
                                        <div wire:ignore x-data="{ value: @entangle('selected_service_id').live }" x-init="
                                            ...
                                            $watch('value', (val) => {
                                                if(val !== ts.getValue()) { ts.setValue(val); }
                                            });
                                        ">
                                            <select x-ref="select" class="w-full bg-white border-slate-200 rounded-xl focus:ring-[#043d8a] focus:border-[#043d8a] text-sm font-bold p-3">
                                                <option value="">Seleccionar Servicio</option>
                                                @foreach($services as $service)
                                                    <option value="{{ $service->id }}">{{ $service->name }} (S/ {{ number_format($$service->price ?? 0, 2) }})</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        @error('selected_service_id') <span class="text-[10px] text-rose-500 font-bold ml-2 mt-1 block">{{ $message }}</span> @enderror
                                    </div>
                                    <div class="w-28">
                                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Precio (S/)</label>
                                        <input type="number" step="0.01" wire:model="selected_price" class="w-full bg-white border-slate-200 rounded-xl focus:ring-[#043d8a] focus:border-[#043d8a] text-sm font-bold p-3 text-center" placeholder="0.00">
                                        @error('selected_price') <span class="text-[10px] text-rose-500 font-bold ml-2 mt-1 block">{{ $message }}</span> @enderror
                                    </div>
                                    <div class="w-24">
                                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Cant.</label>
                                        <input type="number" wire:model="selected_quantity" min="1" class="w-full bg-white border-slate-200 rounded-xl focus:ring-[#043d8a] focus:border-[#043d8a] text-sm font-bold p-3 text-center">
                                    </div>
                                    <button wire:click="addToCart" class="px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-black uppercase text-[10px] tracking-widest transition-all shadow-lg shadow-emerald-500/20 h-[46px]">
                                        Agregar
                                    </button>
                                </div>
                                @error('cart') <div class="text-[10px] text-rose-500 font-bold mb-2">{{ $message }}</div> @enderror
                                    ...
                                    <div class="bg-white rounded-xl border border-slate-100 overflow-hidden mt-6">
                                        <table class="w-full text-left">
                                            <thead class="bg-slate-50 border-b border-slate-100">
                                                <tr>
                                                    <th class="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">Servicio</th>
                                                    <th class="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Cant.</th>
                                            ...
                                            <tbody class="divide-y divide-slate-50">
                                                @foreach($cart as $index => $item)
                                                    <tr>
                                                        <td class="px-4 py-3 font-bold text-slate-700 text-xs">{{ $item['name'] }}</td>
                                                        <td class="px-4 py-3 font-black text-slate-900 text-center text-xs">{{ $item['quantity'] }}</td>
                                                        <td class="px-4 py-3 font-black text-[#043d8a] text-right text-xs">S/ {{ number_format($item['subtotal'], 2) }}</td>
                                                        <td class="px-4 py-3 text-center">
                                                            <button wire:click="removeFromCart({{ $index }})" class="text-rose-400 hover:text-rose-600 p-1.5 rounded-lg hover:bg-rose-50 transition-colors">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 items-end" x-data="{ method: @entangle('payment_method') }">
                                <div>
                                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Cliente</label>
                                    <div wire:ignore x-data="{ value: @entangle('client_id').live }" x-init="
                                        let ts = new TomSelect($refs.select, {
                                            create: false,
                                            sortField: { field: 'text', direction: 'asc' },
                                            onChange: (val) => { value = val }
                                    ...
                                    ">
                                        <select x-ref="select" class="w-full bg-slate-50 border-slate-100 rounded-xl focus:ring-[#043d8a] focus:border-[#043d8a] text-sm font-bold p-3">
                                ...
                                <div class="row-span-2">
                                    <div class="bg-[#043d8a]/5 border border-[#043d8a]/10 rounded-2xl p-6 flex flex-col justify-center h-full relative overflow-hidden">
                                        <div class="absolute -right-4 -top-4 w-24 h-24 bg-[#043d8a]/5 rounded-full"></div>
                                        <label class="relative block text-[10px] font-black text-[#043d8a] uppercase tracking-widest mb-2">Total a Cobrar</label>
                                        <div class="relative text-4xl font-black text-[#043d8a]">
                                ...
                                <div x-show="method == 'credit'" x-cloak x-transition class="animate-in fade-in slide-in-from-top-2 md:col-span-2">
                    ...
                    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden">
                        ...
                        <div class="p-8 space-y-4">
                            <div>
                                ...
                                <input type="text" wire:model="expense_description" placeholder="Ej: Cremas, Toallas, Insumos..." class="w-full bg-slate-50 border-slate-100 rounded-xl focus:ring-[#043d8a] focus:border-[#043d8a] text-sm font-bold p-3">
                            </div>
                                ...
                                <div wire:key="expense-{{ $expense->id }}" class="p-6 flex items-center justify-between hover:bg-slate-50/50 transition-colors {{ $editingExpenseId == $expense->id ? 'bg-blue-50/50' : '' }}">
                                    ...
                                    <div class="flex items-center space-x-4">
                                        <div class="text-right">
                                            <div class="font-black text-rose-600">- S/ {{ number_format($$expense->amount ?? 0, 2) }}</div>
                                            <div class="text-[10px] text-slate-400">{{ optional($expense->created_at)->format('H:i A') }}</div>
                                        </div>
                    ...
                    <table class="w-full text-left">
                        <thead class="bg-slate-50/50 border-b border-slate-100">
                            <tr>
                                ...
                                <th class="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Servicio</th>
                            ...
                            @forelse($sales as $sale)
                                <tr wire:key="sale-{{ $sale->id }}" class="hover:bg-slate-50/50 transition-colors {{ $editingSaleId == $sale->id ? 'bg-blue-50/80' : '' }}">
                                    <td class="px-8 py-5">
                                        ...
                                        <div class="text-[10px] text-slate-400 font-black uppercase tracking-tighter">{{ optional($sale->created_at)->format('H:i A') }}</div>
                                    </td>
                                    <td class="px-8 py-5 font-bold text-slate-700 text-xs">
                                        @if($sale->items && $sale->items->count() > 0)
                                            <div class="space-y-1">
                                                @foreach($sale->items as $item)
                                                    <div class="flex items-center space-x-2">
                                                        <span class="text-[10px] bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full font-black">{{ $item->quantity }}x</span>
                                                        <span class="uppercase">{{ $item->service->name ?? 'Servicio' }}</span>
                                                    </div>
                                                @endforeach
                                            </div>
                                        @else
                                            <span class="uppercase text-slate-400 italic">Sin servicios</span>
                                        @endif
                                    </td>
                                    <td class="px-8 py-5 font-black text-slate-900">S/ {{ number_format($$sale->amount ?? 0, 2) }}</td>
                                    <td class="px-8 py-5 font-bold text-emerald-600">S/ {{ number_format($$sale->paid_amount ?? 0, 2) }}</td>
                                    <td class="px-8 py-5 font-bold text-rose-600">
                                        @if($sale->amount - $sale->paid_amount > 0)
                                            S/ {{ number_format(($sale->amount ?? 0) - ($sale->paid_amount ?? 0), 2) }}
                                        @else
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 px-8 mt-4">
                    <div class="bg-emerald-50 p-6 rounded-3xl border border-emerald-100 shadow-sm relative overflow-hidden group">
                        ...
                        <div class="relative">
                            ...
                            <h3 class="text-2xl font-black text-emerald-700">S/ {{ number_format($$totalPeriodSales ?? 0, 2) }}</h3>
                        </div>
                    </div>
                    <div class="bg-blue-50 p-6 rounded-3xl border border-blue-100 shadow-sm relative overflow-hidden group">
                        ...
                        <div class="relative">
                            ...
                            <h3 class="text-2xl font-black text-[#043d8a]">S/ {{ number_format($$totalPeriodCollected ?? 0, 2) }}</h3>
                        </div>
                    </div>
                    <div class="bg-rose-50 p-6 rounded-3xl border border-rose-100 shadow-sm relative overflow-hidden group">
                        ...
                        <div class="relative">
                            ...
                            <h3 class="text-2xl font-black text-rose-700">S/ {{ number_format($$totalPeriodExpenses ?? 0, 2) }}</h3>
                        </div>
                    </div>
                </div>
            @endif
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors {{ $hasDebt ? 'bg-amber-50/30' : '' }}">
                                <td class="px-8 py-5">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        @endif
                                        @if($hasDebt)
                                            <span class="bg-amber-100 text-amber-700 text-[8px] font-black uppercase px-2 py-0.5 rounded-full border border-amber-200">Deuda: S/ {{ number_format($$pendingAmount ?? 0, 2) }}</span>
                                        @endif
                                    </div>
                                </td>
                                <td class="px-8 py-5 font-bold text-slate-700">S/ {{ number_format($session->starting_cash ?? 0, 2) }}</td>
                                <td class="px-8 py-5 font-black text-slate-900">S/ {{ number_format($session->closing_cash ?? 0, 2) }}</td>
                                <td class="px-8 py-5">
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                    ...
                    <p class="text-lg font-black text-slate-900">S/ {{ number_format($selectedSession->starting_cash ?? 0, 2) }}</p>
                </div>
                ...
                <div class="bg-[#043d8a] p-4 rounded-2xl shadow-sm">
                    ...
                    <p class="text-lg font-black text-white">S/ {{ number_format($selectedSession->closing_cash ?? 0, 2) }}</p>
                </div>
                        ...
                        <table class="w-full text-left">
                            <thead class="bg-slate-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-[9px] font-black uppercase text-slate-400">Servicio</th>
                            ...
                            <tbody class="divide-y divide-slate-50">
                                @foreach($selectedSession->gasSales as $s)
                                    <tr class="text-sm border-b border-slate-50 last:border-0">
                                                ...
                                                <div class="mt-2 space-y-1">
                                                    @foreach($s->payments as $p)
                                                        <div class="flex items-center text-[9px] text-slate-400 bg-slate-50 rounded px-2 py-0.5 w-fit">
                                                            <span class="font-black mr-2">S/ {{ number_format($$p->amount ?? 0, 2) }}</span>
                                                            <span>{{ optional($p->created_at)->format('d/m H:i') }}</span>
                                                        </div>
                                                    @endforeach
                                                </div>
                                            @endif
                                        ...
                                        <td class="px-6 py-4 font-black">S/ {{ number_format($$s->amount ?? 0, 2) }}</td>
                                        <td class="px-6 py-4 font-bold text-emerald-600">S/ {{ number_format($$s->paid_amount ?? 0, 2) }}</td>
                                        <td class="px-6 py-4 font-bold text-rose-600">S/ {{ number_format(($s->amount ?? 0) - ($s->paid_amount ?? 0), 2) }}</td>
                                        <td class="px-6 py-4">
                            ...
                            <tbody class="divide-y divide-slate-50">
                                @foreach($selectedSession->expenses as $e)
                                    <tr class="text-sm">
                                        ...
                                        <td class="px-6 py-4 font-black text-rose-600">S/ {{ number_format($$e->amount ?? 0, 2) }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        @forelse($pendingDebts as $debt)
                            <tr wire:key="debt-{{ $debt->id }}" class="hover:bg-amber-50/20 transition-colors">
                                <td class="px-8 py-5">
                                    <div class="font-bold text-slate-900">{{ optional($debt->created_at)->format('d/m/Y') }}</div>
                                    ...
                                    @if($debt->payments->count() > 0)
                                        <div class="mt-1 flex flex-wrap gap-1">
                                            @foreach($debt->payments as $p)
                                                <span class="text-[8px] bg-slate-100 px-1.5 py-0.5 rounded text-slate-500">S/{{ number_format($$p->amount ?? 0, 2) }}</span>
                                            @endforeach
                                        </div>
                                    @endif
                                </td>
                                <td class="px-8 py-5 font-bold text-slate-700">S/ {{ number_format($$debt->amount ?? 0, 2) }}</td>
                                <td class="px-8 py-5 font-bold text-emerald-600">S/ {{ number_format($$debt->paid_amount ?? 0, 2) }}</td>
                                <td class="px-8 py-5 font-black text-rose-600">S/ {{ number_format(($debt->amount ?? 0) - ($debt->paid_amount ?? 0), 2) }}</td>
                                <td class="px-8 py-5 text-right whitespace-nowrap">
                                    <div class="flex items-center justify-end space-x-2">
...
@push('styles')
    ...
    <style>
        .ts-control { border-radius: 0.75rem; border-color: #e2e8f0; padding: 0.75rem; font-size: 0.875rem; font-weight: 700; background-color: #f8fafc; border-width: 1px; }
        .ts-control.focus { border-color: #043d8a; box-shadow: 0 0 0 1px #043d8a; }
        .ts-dropdown { border-radius: 0.75rem; overflow: hidden; font-size: 0.875rem; font-weight: 500; }
        .ts-dropdown .option:hover, .ts-dropdown .active { background-color: #f1f5f9; color: #0f172a; }
        .ts-wrapper.single .ts-control { padding: 0.75rem; }
        .ts-wrapper.single .ts-control::after { border: none !important; content: '▼' !important; font-size: 10px; color: #94a3b8; right: 15px; margin-top: -6px; }
    </style>
@endpush
@push('scripts')
