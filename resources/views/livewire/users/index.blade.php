<div class="space-y-10">
    ...
    <div class="md:flex md:items-center md:justify-between px-4">
        <div class="flex-1 min-w-0">
            <h2 class="text-3xl md:text-4xl font-extrabold tracking-tight text-[#0f172a]">
                Gestión de <span class="text-primary font-black">Usuarios</span>
                ...
                <tr class="hover:bg-slate-50/80 transition-colors group">
                    ...
                    <td class="px-8 py-5 whitespace-nowrap text-right text-sm font-medium">
                        <div class="flex items-center justify-end space-x-2">
                            ...
                            @if($user->id !== auth()->id())
                            <button wire:click="confirmDelete({{ $user->id }})" class="text-rose-600 hover:text-rose-900 bg-rose-50 hover:bg-rose-100 p-2 rounded-lg transition-colors">
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
        <div class="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl border border-slate-100 overflow-hidden">
            <div class="px-8 py-6 flex items-center justify-between border-b border-slate-50">
                <h3 class="text-xl font-black text-slate-900 tracking-tight">Confirmar <span class="text-rose-600">Eliminación</span></h3>
                <button wire:click="cancelDelete" class="text-slate-400 hover:text-slate-600 transition-colors">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>

            <div class="p-8 space-y-6 text-center">
                <div class="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-rose-50 text-rose-600">
                ...
                <div class="space-y-3">
                    <p class="text-base font-bold text-slate-900">
                        ¿Estás seguro de que deseas eliminar a <span class="text-rose-600 font-black">{{ $userNameToDelete }}</span>?
                    </p>
                    <p class="text-sm text-slate-500 font-medium leading-relaxed">
                        El usuario será eliminado del sistema y ya no tendrá acceso. Sin embargo, todos sus registros relacionados (clientes, llamadas, asistentes) se conservarán intactos en la base de datos por seguridad y referencia histórica.
                    </p>
                </div>

                <div class="flex space-x-3 pt-2">
                    <button type="button" wire:click="cancelDelete" class="flex-1 px-6 py-3 border border-slate-200 rounded-2xl text-sm font-bold text-slate-700 hover:bg-slate-50 transition-all">
                        Cancelar
                    </button>
                    <button type="button" wire:click="delete" class="flex-1 px-6 py-3 bg-rose-600 text-white rounded-2xl shadow-lg shadow-rose-500/20 text-sm font-bold hover:bg-rose-700 transition-all uppercase tracking-widest">
                        Sí, eliminar
                    </button>
                </div>
            </div>
