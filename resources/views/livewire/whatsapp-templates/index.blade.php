<div class="space-y-10">
    ...
    <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden mx-4">
        <table class="min-w-full divide-y divide-slate-100">
            <thead class="bg-slate-50/50">
                <tr>
                    <th class="px-8 py-5 text-left text-xs font-black text-slate-400 uppercase tracking-widest">Título</th>
                    <th class="px-8 py-5 text-left text-xs font-black text-slate-400 uppercase tracking-widest">Mensaje</th>
                    @if(!auth()->user()->isAssistant())
            ...
            <tbody class="divide-y divide-slate-100 bg-white">
                ...
                <tr class="hover:bg-slate-50/80 transition-colors group">
                    <td class="px-8 py-5 whitespace-nowrap text-sm font-black text-slate-900">{{ $template->title }}</td>
                    ...
                    <td class="px-8 py-5 whitespace-nowrap text-right text-sm font-medium">
                        <div class="flex items-center justify-end space-x-2">
                            <button wire:click="editTemplate({{ $template->id }})" class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 hover:bg-indigo-100 p-2 rounded-lg transition-colors">
                            ...
                            <button wire:click="delete({{ $template->id }})" wire:confirm="¿Estás seguro?" class="text-rose-600 hover:text-rose-900 bg-rose-50 hover:bg-rose-100 p-2 rounded-lg transition-colors">
                ...
                @empty
                <tr>
                    <td colspan="3" class="px-8 py-10 text-center text-slate-400 font-medium">No hay plantillas creadas.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
        <div class="bg-white rounded-[2.5rem] w-full max-w-lg shadow-2xl border border-slate-100 overflow-hidden">
            <div class="px-8 py-6 flex items-center justify-between border-b border-slate-50">
                <h3 class="text-xl font-black text-slate-900 tracking-tight">Nueva <span class="text-indigo-600">Plantilla</span></h3>
            ...
            <form wire:submit.prevent="save" class="p-8 space-y-5">
                <div>
                    <label class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Título de la Plantilla</label>
                    <input type="text" wire:model="title" placeholder="Ej: Bienvenida, Recordatorio Pago..." class="w-full px-4 py-3 bg-slate-50 border-transparent rounded-xl text-sm font-bold text-slate-900 focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all">
                    @error('title') <span class="text-rose-500 text-[10px] font-bold mt-1">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Contenido del Mensaje</label>
                    <textarea wire:model="description" rows="5" class="w-full px-4 py-3 bg-slate-50 border-transparent rounded-xl text-sm font-bold text-slate-900 focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all resize-none" placeholder="Escribe el mensaje aquí..."></textarea>
                    @error('description') <span class="text-rose-500 text-[10px] font-bold mt-1">{{ $message }}</span> @enderror
                    <p class="mt-2 text-[10px] text-slate-400 font-medium leading-tight">
                        * Este mensaje será precargado al seleccionar la plantilla en el perfil del cliente.
                    </p>
                </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
        <div class="bg-white rounded-[2.5rem] w-full max-w-lg shadow-2xl border border-slate-100 overflow-hidden">
            <div class="px-8 py-6 flex items-center justify-between border-b border-slate-50">
                <h3 class="text-xl font-black text-slate-900 tracking-tight">Editar <span class="text-indigo-600">Plantilla</span></h3>
                <button wire:click="$set('showEditModal', false)" class="text-slate-400 hover:text-slate-600 transition-colors">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>

            <form wire:submit.prevent="updateTemplate" class="p-8 space-y-5">
                <div>
                    <label class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Título de la Plantilla</label>
                    <input type="text" wire:model="title" class="w-full px-4 py-3 bg-slate-50 border-transparent rounded-xl text-sm font-bold text-slate-900 focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all">
                    @error('title') <span class="text-rose-500 text-[10px] font-bold mt-1">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Contenido del Mensaje</label>
                    <textarea wire:model="description" rows="5" class="w-full px-4 py-3 bg-slate-50 border-transparent rounded-xl text-sm font-bold text-slate-900 focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all resize-none"></textarea>
                    @error('description') <span class="text-rose-500 text-[10px] font-bold mt-1">{{ $message }}</span> @enderror
                </div>
